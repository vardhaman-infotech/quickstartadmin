...
{
    public partial class rpt_timeAndExpDetailByClientAndProject : System.Web.UI.Page
    {
        ...
        protected void fillgrid()
        {
            ...
            objts.type = "";
            objts.id = "";
            objts.taskType = droptasktype.Text;
        ...
        private void f_ReportLoad()
        {
            try
            {
                ...
                if(chkcost.Checked)
                ReportViewer1.LocalReport.ReportPath = p + "\\client\\rpt_clientTimeandExp_new.rdlc";
                else
                    ReportViewer1.LocalReport.ReportPath = p + "\\client\\rpt_clientTimeandExp_withoutcost.rdlc";
                ...
                if (Request.QueryString["report"] != null)
                {
                    param[3] = new ReportParameter("reportname", Request.QueryString["report"].ToString(), true);
                }
                else
                {
                    if(chkcost.Checked)
                    param[3] = new ReportParameter("reportname", "Time & Expense Report by Client, Project & Employee (With Cost)", true);
                    else
                        param[3] = new ReportParameter("reportname", "Time & Expense Report by Client, Project & Employee", true);
                }

                param[4] = new ReportParameter("reportfilter", "Report From " + objts.from + " To " + objts.to, true);
                ...
                if (Request.QueryString["report"] != null)
                {
                    ReportViewer1.LocalReport.DisplayName = Request.QueryString["report"].ToString();
                }
                else {
                    ReportViewer1.LocalReport.DisplayName = "Client Time and Expenses";
                }
            }
            catch (Exception ex)
            {
