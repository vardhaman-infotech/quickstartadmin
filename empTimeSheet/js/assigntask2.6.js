...
function savereward(id) {
    ...
    if (charindex != -1) {
        ...
        var comments = "";
        if (document.getElementById(id + "txtcomments")) {
...
function SaveAssignTask() {
    ...
    var newid = "", projectid = "", taskid = "", remark = "", budhours = "";
    ...
    for (var i = 1; i <= id; i++) {
        ...
        newid = "txtbudhrs" + i;
        budhours = budhours + document.getElementById(newid).value + "#";
    ...
    var args = { empid: empid, date: date, projectid: projectid, taskid: taskid, companyid: companyid, createdby: createdby, hours: hours, remark: remark, budgethrs: budhours };
    $.ajax({
...
function bindassignedtasks(id) {
    if (isrun == 0) {
        ...
        var tblheader = '<thead><tr class="gridheader"><th style="width: 2.4%;">&nbsp;</th><th style="width: 7.8%;"> AssignDate </th><th  style="width:9.7%;"> Employee</th>' +
                                    '<th style="width:7.2%;"> ProjectID</th> <th style="width:10%;">TaskID</th> <th style="width:13%"> Remark</th> <th style="width: 5.4%;"> BHours</th><th style="width: 61px;">TimeTaken' +
                                   '</th><th style="width: 40px;"> Status</th><th style="width: 5%"> Manager</th><th style="width: 8.5%;"> LastUpdated </th><th style="width: 60px;"> Grade</th>' +
                                    '<th style="width: 30px;"> &nbsp; </th><th style="width: 30px;"> &nbsp;</th></tr></thead>';
        isrun = 1;
        ...
        $.ajax({
            ...
            success: function (data) {
                //Check length of returned data, if it is less than 0 it means there is some status available
                if (data.d != "failure") {
                    $("#dgnews").colResizable({ disable: true });
                    ...
                    for (var i in json)
                    {
                        ...
                        if (i == json.length - 1) {
                            ...
                            txtremark = "<input tyle='text' id='" + json[i].nid + "_txtremark'  onchange='saveRemark(this.id);' class='form-control' value='" + json[i].remark + "' />";
                            ddlgrade = "<select style='width:80px;' onchange='savereward(this.id);' title='Task Grade can be given after completion of task' id='" + json[i].nid + "_ddlgrade' class='form-control'>" +
                                                "<option value=''></option>" +
                            ...
                            if (addrole == "1") {
                                if (status != "completed") {
                                    ddlgrade = "<select style='width:80px;' disabled='disabled' onchange='savereward(this.id);' title='Task Grade can be given after completion of task' id='" + json[i].nid + "_ddlgrade' class='form-control'>" +
                            ...
                            $('#dgnews tbody').append("<tr><td>" + imgassignedby + "</td><td>" + json[i].date + "</td><td><span title='" + json.loginid + "'>" + json[i].empname +
                "</span></td><td><span title='" + json.projectname + "'>" + json[i].projectCode + "</span></td><td>" + json[i].taskcodename + ':' + json[i].task +
                "</td><td>" + txtremark + hiddenfieldnid + "</td><td>" + json[i].TbujHrs + "</td><td><span id='" + json[i].nid + "_ltrhours'>" + json[i].totalhour + "</span></td><td><span id='" + json[i].nid + "_ltrstatus'>" + json[i].status.toString()
                + "</span></td><td><span title='" + json.managername + "'>" + json[i].managerloginid + "</span></td><td><span id='" + json[i].nid + "_ltrLastModifiedDate'>" + LastModifiedDate + "</span></td><td>" + ddlgrade + "</td>" +
               "<td>" + lbtnstatus + "</td><td>" + linkdelete + "</td></tr>");
...
function bindtaskvalue(id, value) {
    ...
    document.getElementById("txtbudhrs" + rownum).value = taskarr[3];
...
function addrow() {
    ...
    var cellbhours = row.insertCell(3);
    var celldelete = row.insertCell(4);

    cellproject.innerHTML = "<input type='text' id='ddlproject" + newsno + "' class='form-control form-control1'  /> <input type='hidden' id='hidproject" + newsno + "' />";
    celltask.innerHTML = "<input type='text' id='ddltask" + newsno + "' class='form-control form-control1' onchange='bindtaskvalue(this.id,this.value);'   /><input type='hidden' id='hidtask" + newsno + "' />";
    celldes.innerHTML = "<input type='text' id='txtdesc" + newsno + "' class='form-control form-control1'  onblur='removeSpecialCh(this.id,event);' onkeypress='blockSpecialCh(this,event);'  />";
    cellbhours.innerHTML = "<input type='text' id='txtbudhrs" + newsno + "' class='form-control' maxlength='4' onkeypress='TS_blockNonNumbers(this, event, true, false,0);'  onblur='extractNumber(this,2,false);calhours();' onkeyup='extractNumber(this,2,false);'  />";
