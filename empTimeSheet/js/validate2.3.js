...
function comparedatewithrange(id, from, to, range) {
    ...
    if (val != "" && val != "__/__/____") {
        if (!isDate(val)) {
            ...
            if (fromval != "" && toval != "" && (document.getElementById(range).value == "Custom")) {
...
function fncomparedate(val, id) {
    if (val != "" && val != "__/__/____") { }
    var joinDate_byEmp = document.getElementById("hdnjoinDate_byEmp").value
    var getTimeEntry = document.getElementById(id).value;
    if (val != "" && val != "__/__/____") {
        if (!isDate(val)) {
            alert('Invalid date format, date must be in mm/dd/yyyy format');
            document.getElementById(id).value = "";
            ...
            if (joinDate_byEmp != "" && getTimeEntry != "") {

                var d1 = new Date(joinDate_byEmp);
                var d2 = new Date(getTimeEntry);
                if (d1 > d2) {
                    alert("You cannot fill time sheet before joining dates.");
                    event.preventDefault();
                    document.getElementById(id).value = "";
                    $("#ctl00_ContentPlaceHolder1_hdnflddate0").val('');
                }
                else {
                    $("#ctl00_ContentPlaceHolder1_hdnflddate0").val(getTimeEntry);
                    document.getElementById(id).value =$("#ctl00_ContentPlaceHolder1_hdnflddate0").val()
                }
            }
        }
    }
